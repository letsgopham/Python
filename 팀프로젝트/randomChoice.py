...
def random_choice(random_list):
    ...
    def Choice():
        if choice in response:
            ...
            name = selected2.get('가게 이름')
            category = selected2.get('카테고리')
            address = selected2.get('주소')
            distance = selected2.get('거리')
            phone = selected2.get('전화번호')
            Kakao_URL = selected2.get('카카오맵 바로가기')
